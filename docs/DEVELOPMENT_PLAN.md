...
- declarative external widgets (v1 strict data-only document + normalization contract, bounded read-only Application Support loader, startup-only atomic registration, and sanitized startup-health model implemented; watch/install UX deferred)
...
2. Validate startup-only external widget registration on real app launches, keeping live reload/watch semantics deferred.
