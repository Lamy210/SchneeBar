...
- declarative external widgets (v1 strict data-only document + normalization contract, bounded read-only Application Support loader, startup-only atomic registration, sanitized startup-health model, and read-only Settings health presentation implemented; watch/install UX deferred)
