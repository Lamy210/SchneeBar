...
func applicationSupportFailureClassificationIsSanitized(
    error: ExternalWidgetDirectoryLoaderError,
    expected: ExternalWidgetStartupFailureReason
) {
    #expect(
        ExternalWidgetStartupRegistrar
            .classifyApplicationSupportFailure(error) == expected
    )
}

@Test
func unknownStartupFailureIsSanitizedWithoutRetainingOriginalError() {
    #expect(
        ExternalWidgetStartupRegistrar.classifyApplicationSupportFailure(
...
@Test
func providerCollisionIsSanitizedAndLeavesNativeProviderUntouched() async throws {
    ...
    let registrar = ExternalWidgetStartupRegistrar(
        loadDefinitions: { [definition] },
        classifyFailure: ExternalWidgetStartupRegistrar
            .classifyApplicationSupportFailure
    )
...
@Test
func successfulResultExposesOnlyWidgetCount() async throws {
    let definitions = [
