...
public struct ExternalWidgetDirectoryLoader: Sendable {
    ...
    private func openRootDirectory() throws -> Int32? {
        let ownerDirectoryURL = rootURL.deletingLastPathComponent()
        let trustedParentURL = ownerDirectoryURL.deletingLastPathComponent()
        let ownerName = ownerDirectoryURL.lastPathComponent
        let rootName = rootURL.lastPathComponent

        guard isSafeDirectoryComponent(ownerName),
              isSafeDirectoryComponent(rootName)
        else {
            throw ExternalWidgetDirectoryLoaderError.unsafeRoot
        }

        let flags = O_RDONLY | O_CLOEXEC | O_DIRECTORY | O_NOFOLLOW
        let trustedParentDescriptor = trustedParentURL.path.withCString {
            Darwin.open($0, flags)
        }
        guard trustedParentDescriptor >= 0 else {
    ...
    private func isSafeDirectoryComponent(_ value: String) -> Bool {
        !value.isEmpty
            && value != "."
            && value != ".."
            && !value.contains("/")
            && value.utf8.count <= Self.maximumFilenameBytes
